...
## 5. Loss function w sensivity analysis
### Case 1
```{r}
# select all unique solutions given p0p1
...
# calculate score for w from 0.01 to 0.99 using for loop
score_datas = c()
for (w in seq(0.0,1.0,0.05)){
  score = apply(case1, 1, loss_func_1s, alpha=0.20, beta=0.20, w=w)
  score_data = cbind(data.frame(w=rep(w,nrow(case1)), loss=score), case1)
  # select row with the minimum score
  score_data = score_data %>% arrange(score) %>% filter(row_number() %in% c(1))
  # append to score_datas
  score_datas = rbind(score_datas, score_data)
}

score_datas
write.csv(score_datas, 'manuscript/tbl_S1_case1.csv', row.names = F)
...
score_datas = c()
for (w in seq(0.0,1.0,0.05)){
  score = apply(case2, 1, loss_func_1s, alpha=0.20, beta=0.20, w=w)
  score_data = cbind(data.frame(w=rep(w,nrow(case2)), loss=score), case2)
  # select row with the minimum score
  score_data = score_data %>% arrange(score) %>% filter(row_number() %in% c(1))
  # append to score_datas
  score_datas = rbind(score_datas, score_data)
...
score_datas
write.csv(score_datas, 'manuscript/tbl_S1_case2.csv', row.names = F)
...
```
### New figure S1 and table S1
```{r}
# for unique pairs of p0 and p1, do the above
score_datas = c()
for (w in seq(0.0,1.0,0.05)){
  score = apply(tdr_alpha20_1s2by2_alphaonly, 1, loss_func_1s, alpha=0.20, beta=0.20, w=w)
  score_data = cbind(data.frame(w=rep(w,nrow(tdr_alpha20_1s2by2_alphaonly)), loss_score=score), tdr_alpha20_1s2by2_alphaonly)
  score_data = score_data %>% # filter(power>=0.75) %>%
  group_by(p0,p1,w,gammax) %>%
  arrange(p0,p1,loss_score,expicmax) %>%
  filter(row_number() %in% c(1)) %>%
  group_by(p0,p1,w) %>%
  arrange(p0,p1,loss_score) %>%
...
score_datas = score_datas %>% arrange(p0,p1,w) %>% select(p0,p1,w,loss_score,N,power,gammax,expicmax)
head(score_datas)
# write.csv(score_datas, 'manuscript/tbl_S1_1s2by2_alpha20only_allcases.csv', row.names = F) 
```


```{r}
score_datas$case <- factor(paste("p0:", score_datas$p0, "p1:", score_datas$p1))

# Randomly select 10 cases
set.seed(512)
cases <- unique(score_datas$case)
case_rnd <- sample(cases, 10)
score_datas_rnd <- score_datas[score_datas$case %in% case_rnd, ]

# Prepare data for faceting
score_datas_rnd <- tidyr::pivot_longer(
  score_datas_rnd, 
  cols = c(N, power), 
  names_to = "variable", 
  values_to = "value"
...
# Plot with facets
ggplot(score_datas_rnd, aes(x = w, y = value, color = case)) +
  geom_line(size = 0.5) +
  facet_wrap(~variable, scales = "free_y", labeller = labeller(variable = c(N = "N", power = "Power"))) +
  scale_color_discrete() +
  labs(x = "w", y = "", title = "Sample size and power by w") +
  theme_light() +
  theme(plot.title = element_text(face = "bold", size = rel(1), hjust = 0.5, vjust=0.5),
        axis.text.x = element_text(angle = 60, vjust=0.5, size=rel(0.8)),
        axis.title.y = element_text(angle=90,vjust =2, size=rel(0.8)),
