##########################################################################
#            Three-regional Dual-criterion Two-stage 3 by 2              #
##########################################################################
# -----------------------------------------------------------------------#
...
P_LIST_ID = as.numeric(ARGS[1])
print(paste("Paralell job: ", P_LIST_ID))
...
# get stage 2 metrics
getstg2prob = function(p0,p1,nc1,nc2,m1,a1,b1L,b2L,m2L,call,L=1e-9,k=1) {
  ...
  nb2L = length(b2L)
  nm2L = length(m2L)
  pb1b2m2 = array(0, c(nb1L,nb2L,nm2L))
  ne1 = nc1*k
  ne2 = nc2*k
  ...
  for (ec3 in ec3L) {
    ...
    for (ec2 in ec2L) {
      ...
      if (call=='power' || call=='alpha') {
        pb1b2m2 = pb1b2m2 + outer(outer(999>b1L, Z>=b2L),E>=m2L) * psav
      }
      if (call=='type2' || call=='nogo') {
        pb1b2m2 = pb1b2m2 + outer(outer(Z<=b1L, 999>b2L),999>m2L) * psav + outer(outer(Z>b1L, Z<b2L),E<m2L) * psav
      } else if (call == 'eta1' || call=='gam1') {
        pb1b2m2 = pb1b2m2 + outer(outer(Z>b1L, Z<b2L), E>=m2L)*psav
  ...
  return(pb1b2m2) # 3 dimension b1*b2*m2
}

####---------------------Main Function----------------------####
getTDRfunc2s3by2 = function(x, k=1, FILENAME=FILENAME) {
  p0=x['p0'][[1]]
  p1=x['p1'][[1]]
  ...
  while (nc2 <= nc2max) { # nc2 loop step by 4
    ...
    b1min = max(-NC, floor(p0*(NE-k*NC) - 4*se0))
    b1max = min(NE, ceiling(p0*NE-p0*k*NC + 2*se0))
    
    b2min = floor(p0*(NE-k*NC))
    ...
    # control alpha
    alpL = getstg2prob(p0,p0,nc1,nc2,m1,a1,b1L,b2L,m2L,'alpha',LL,k)
    alpLidx = which(alpL<=alphamax)
    
    alpLsi = which(alpL <= alphamax, arr.ind = T)
    constrainb1 = b1L[alpLsi[,1]]
    constrainb2 = b2L[alpLsi[,2]]
    constrainm2 = m2L[alpLsi[,3]]
    constrain = (constrainb1 < constrainb2) & (constrainb2 < constrainm2)
    alpLsicontrain = alpLsi[constrain,]
    nm2 = length(m2L)
    nb1 = length(b1L)
    nb2 = length(b2L)
    if (length(alpLsicontrain)==3) {
      constrainx = (alpLsicontrain[3]-1)*nb1*nb2 + (alpLsicontrain[2]-1)*nb1+alpLsicontrain[1]
    } else {
      constrainx = (alpLsicontrain[,3]-1)*nb1*nb2 + (alpLsicontrain[,2]-1)*nb1+alpLsicontrain[,1]
    }
    ...
    if (length(xd) > 0) {  # control beta
      betaL = getstg2prob(p0,p1,nc1,nc2,m1,a1,b1L,b2L,m2L,'type2',LL,k) 
      betaLidx = which(betaL<=betamax)
      xd=intersect(xd, betaLidx)
      
      if (length(xd) > 0) { # control alpha1
        alp1L = getstg2prob(p01,p01,nc1,nc2,m1,a1,b1L,b2L,m2L,'alpha',LL,k)
        alp1Lidx = which(alp1L<=alpha1max)
        xd = intersect(xd, alp1Lidx)
        
        if (length(xd) > 0) { # control alpha2
          alp2L = getstg2prob(p02,p02,nc1,nc2,m1,a1,b1L,b2L,m2L,'alpha',LL,k)
          alp2Lidx = which(alp2L<=alpha2max)
          xd = intersect(xd, alp2Lidx)
          
          if (length(xd) > 0) { # control alpha max
            for (p0s in p0.ci) { 
              alpmaxL = getstg2prob(p0s,p0s,nc1,nc2,m1,a1,b1L,b2L,m2L,'alpha',LL,k)
              alpmaxLidx = which(alpmaxL<=alphamaxmax)
              xd=intersect(xd,alpmaxLidx)
            }
            if (length(xd) > 0) { # control pwr
              pwrL = getstg2prob(p0,p1,nc1,nc2,m1,a1,b1L,b2L,m2L,'power',LL,k) # probability list
              pwrLidx = which(pwrL >= pwrmin) # index
              xd = intersect(pwrLidx, xd)
              
              if (length(xd) > 0) { # control eta
                eta1L = getstg2prob(p0,p0,nc1,nc2,m1,a1,b1L,b2L,m2L,'eta1',LL,k)
                eta2L = getstg2prob(p0,p0,nc1,nc2,m1,a1,b1L,b2L,m2L,'eta2',LL,k)
                etaL = eta1L + eta2L
                etaLidx = which(etaL<=etamax)
                xd=intersect(xd, etaLidx)
                
                if (length(xd) > 0) {
                  gam1L = getstg2prob(p0,p1,nc1,nc2,m1,a1,b1L,b2L,m2L,'gam1',LL,k) 
                  gam2L = getstg2prob(p0,p1,nc1,nc2,m1,a1,b1L,b2L,m2L,'gam2',LL,k) 
                  gamL = gam1L + gam2L
                  ...
                  if (length(xd) > 0) {
                    ...
                    nogoL = getstg2prob(p0,p0,nc1,nc2,m1,a1,b1L,b2L,m2L,'nogo',LL,k)
    ...
    if (soln==3 || nc2==nc2s) {
      ...
      if (length(iam) > 0) {
        m2idx = floor((iam-0.01)/(nb1*nb2)) + 1
        b2idx = floor((iam - (m2idx-1)*nb1*nb2 - 0.01)/nb1) + 1
        b1idx = iam - (m2idx-1)*nb1*nb2 - (b2idx-1)*nb1
        b1 = b1L[b1idx]
        b2 = b2L[b2idx]
        m2 = m2L[m2idx]
        ...
        alphamaxvalue = -0.01
        for (p0s in p0.ci) { 
          alpmaxL = getstg2prob(p0s,p0s,nc1,nc2,m1,a1,b1,b2,m2,'alpha',LL,k)
          alphamaxvalue = max(alphamaxvalue, alpmaxL[1])
        }
        
        search_result = c(p0,p1,nc1,ne1,a1,m1,nc2,ne2,b1,b2,m2,
        ...
        sink(paste('results_2s3by2/',FILENAME, sep=''), append=T)
        cat('\n')
...
####---------------------Experiment---------------------####
tdr_2s_3by2 = function(FILEPREFIX, alphamax, alpha1max, alpha2max, alphamaxmax, betamax, pwrmin, eta, gam, expic, ci=0.9,k=1) {
  FILENAME = paste(FILEPREFIX,'_eta', round(eta*100),'_gam', round(gam*100), '_ic',round(expic*100), '_confidence',round(ci*100), '.csv', 
                   sep='')
  
  print(FILENAME)
  # tryCatch({
  #   dat = read.csv(paste('results_2s3by2/',FILENAME, sep=''), skip = 2)
  # }, error=function(e){
  #   dat <<- data.frame(c=1)
  ...
  #                  expicmax=expicmax,
  #                  ci=ci)
  # dat = data.frame(p0=c(0.05,0.15,0.25,0.35,0.50,0.70,0.20,0.20,0.30,0.30,0.40,0.40,0.50,0.50,0.60,0.60),
  #                  p1=c(0.20,0.30,0.40,0.50,0.65,0.85,0.40,0.45,0.50,0.55,0.60,0.65,0.70,0.75,0.80,0.85),
  #                  es0min=0.50,
  #                  es1max=0.05,
  #                  alphamax=alphamax,
  #                  betamax=betamax,
  ...
  dat = data.frame(p0=c(0.10,0.10,0.10,0.20,0.20,0.20,0.35,0.35,0.35,0.30,0.30,0.40,0.40,0.50,0.50,0.55,0.55,0.60,0.65,0.70),
                   p1=c(0.25,0.30,0.35,0.35,0.40,0.45,0.50,0.55,0.60,0.45,0.50,0.60,0.65,0.65,0.70,0.70,0.75,0.85,0.85,0.85),
                   es0min=0.50,
                   es1max=0.05,
                   alphamax=alphamax,
                   betamax=betamax,
  ...
  # write data
  sink(paste('results_2s3by2/',FILENAME, sep=''), append=F)
  cat('\n')
  cat(FILENAME)
  cat('\n')
  cat(paste0(c('p0', 'p1', 'nc1','ne1','a1','m1','nc2','ne2','b1','b2','m2',
  ...
  for (i in 1:nrow(dat)) {
    print(i)
    tic()
    s2 = getTDRfunc2s3by2(dat[i,],k=k, FILENAME=FILENAME)
    toc()
    # zTDR_2s_results = rbind(TDR_2s_results, s2)
  }
  # } else {
  #   print('haha it was done!')
  # }
...
# search_grid = expand.grid(p0=rep(seq(0.05,0.70,0.05),4),
#                           p1=c(seq(0.15,0.80,0.05), seq(0.20,0.85,0.05),seq(0.25,0.90,0.05),seq(0.30,0.95,0.05)),
#                           expic = seq(0.05,0.25,0.05), gam=seq(0.05,0.15,0.01))
# 
# p0_p1 = data.frame(p0=rep(seq(0.05,0.70,0.05),4),
#                    p1=c(seq(0.15,0.80,0.05), seq(0.20,0.85,0.05),seq(0.25,0.90,0.05),seq(0.30,0.95,0.05)))
search_grid = expand.grid(expic = seq(0.05,0.25,0.05), gam=seq(0.05,0.15,0.01))


# p0 = search_grid$p0[P_LIST_ID]
# p1 = search_grid$p1[P_LIST_ID]

if (P_LIST_ID %in% seq(1,55)) {
  gam = search_grid$gam[P_LIST_ID]
  expic = search_grid$expic[P_LIST_ID]
  fileprefix1 = 'TDR_alpha20_2s_3by2_alphaonly'
  tdr_2s_3by2(FILEPREFIX=fileprefix1, 
...
# if (P_LIST_ID %in% seq(56,110)) {
#   gam = search_grid$gam[P_LIST_ID-55]
#   expic = search_grid$expic[P_LIST_ID-55]
#   fileprefix2 = 'TDR_alpha10_2s_3by2_alphaonly'
#   tdr_2s_3by2(FILEPREFIX=fileprefix2, 
...
# }
# 
# if (P_LIST_ID %in% seq(111,165)) {
#   gam = search_grid$gam[P_LIST_ID-55*2]
#   expic = search_grid$expic[P_LIST_ID-55*2]
#   fileprefix3 = 'TDR_alpha20_2s_3by2_alphamax30'
#   tdr_2s_3by2(FILEPREFIX=fileprefix3, 
#               alphamax=0.20, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.20, 
#               betamax=0.20, pwrmin=0.75,
#               eta=0.99, gam=gam, expic=expic, ci=0.30,
#               k=1) }
# 
# if (P_LIST_ID %in% seq(166,220)) {
#   gam = search_grid$gam[P_LIST_ID-55*3]
#   expic = search_grid$expic[P_LIST_ID-55*3]
#   fileprefix4 = 'TDR_alpha10_2s_3by2_alphamax30'
#   tdr_2s_3by2(FILEPREFIX=fileprefix4, 
...
# if (P_LIST_ID == 1) {
# fileprefix = 'TDR_alpha20_2s_3by2_alphaonly'
# 
# for (expic in seq(0.05,0.25,0.05)) {
#   for (gam in seq(0.01,0.15,0.01)) {
#     tdr_2s_3by2(FILEPREFIX=fileprefix, 
#                 alphamax=0.20, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.99, 
#                 betamax=0.20, pwrmin=0.75,
#                 eta=0.99, gam=gam, expic=expic, ci=0.90,
#                 k=1)
...
# 
# if (P_LIST_ID == 2) {
# fileprefix = 'TDR_alpha10_2s_3by2_alphaonly'
# 
# for (expic in seq(0.05,0.25,0.05)) {
#   for (gam in seq(0.01,0.15,0.01)) {
#     tdr_2s_3by2(FILEPREFIX=fileprefix, 
...
# 
# if (P_LIST_ID == 3) {
# fileprefix = 'TDR_alpha20_2s_3by2_alphamax30'
# 
# for (expic in seq(0.05,0.25,0.05)) {
#   for (gam in seq(0.01,0.15,0.01)) {
#     tdr_2s_3by2(FILEPREFIX=fileprefix, 
#                 alphamax=0.20, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.2, 
#                 betamax=0.20, pwrmin=0.75,
#                 eta=0.99, gam=gam, expic=expic, ci=0.30,
#                 k=1)
#   }
...
# 
# if (P_LIST_ID == 4) {
# fileprefix = 'TDR_alpha10_2s_3by2_alphamax30'
# 
# for (expic in seq(0.05,0.25,0.05)) {
#   for (gam in seq(0.01,0.15,0.01)) {
#     tdr_2s_3by2(FILEPREFIX=fileprefix, 
