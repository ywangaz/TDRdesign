##########################################################################
#            Three-regional Dual-criterion Two-stage 2 by 2              #
##########################################################################
# -----------------------------------------------------------------------#
...
getstg2prob2by2 = function(p0,p1,nc1,nc2,m1,a1,sL,m2L,call,L=1e-9,k=1) {
...
getTDRfunc2s2by2 = function(x, k=1, FILENAME=FILENAME) {
  ...
  while (nc2 <= nc2max) { # nc2 loop step by 4
    ...
    # control alpha
    alpL = getstg2prob2by2(p0,p0,nc1,nc2,m1,a1,sL,m2L,'alpha',LL,k)
              ...
              if (length(xd) > 0) { # control eta
                ...
                if (length(xd) > 0) {
                  ...
                  if (length(xd) > 0) {
                    ...
                    nogoL = getstg2prob2by2(p0,p0,nc1,nc2,m1,a1,sL,m2L,'nogo',LL,k)
    ...
    if (soln==3 || nc2==nc2s) {
      ...
      if (length(iam) > 0) {
        ...
        alphamaxvalue = -0.01
        for (p0s in p0.ci) { 
          alpmaxL = getstg2prob2by2(p0s,p0s,nc1,nc2,m1,a1,s,m2,'alpha',LL,k)
...
####---------------------Experiment---------------------####
tdr_2s_2by2 = function(FILEPREFIX, alphamax, alpha1max, alpha2max, alphamaxmax, betamax, pwrmin, eta, gam, expic, ci=0.9,k=1) {
...
search_grid = expand.grid(expic = seq(0.11,0.19,0.01), gam=c(0.05, 0.10, 0.15))

gam = search_grid$gam[P_LIST_ID]
expic = search_grid$expic[P_LIST_ID]

fileprefix1 = 'TDR_alpha20_2s_2by2_alphaonly'
tdr_2s_2by2(FILEPREFIX=fileprefix1,
            alphamax=0.20, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.99,
            betamax=0.20, pwrmin=0.75,
            eta=0.99, gam=gam, expic=expic, ci=0.90,
            k=1)

# search_grid = expand.grid(expic = seq(0.05,0.30,0.05), gam=seq(0.05,0.20,0.01))
# 
# gam = search_grid$gam[P_LIST_ID]
# expic = search_grid$expic[P_LIST_ID]
# 
...
#             eta=0.99, gam=gam, expic=expic, ci=0.90,
#             k=1)
# fileprefix2 = 'TDR_alpha10_2s_2by2_alphaonly'
# tdr_2s_2by2(FILEPREFIX=fileprefix2, 
#             alphamax=0.10, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.99, 
#             betamax=0.10, pwrmin=0.85,
#             eta=0.99, gam=gam, expic=expic, ci=0.90,
#             k=1)
# fileprefix3 = 'TDR_alpha20_2s_2by2_alphamax30'
# tdr_2s_2by2(FILEPREFIX=fileprefix3, 
#             alphamax=0.20, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.20, 
#             betamax=0.20, pwrmin=0.75,
#             eta=0.99, gam=gam, expic=expic, ci=0.30,
#             k=1)
# fileprefix4 = 'TDR_alpha10_2s_2by2_alphamax30'
# tdr_2s_2by2(FILEPREFIX=fileprefix4, 
...
# if (P_LIST_ID == 1) {
#   fileprefix = 'TDR_alpha20_2s_2by2_alphaonly'
#   
#   for (expic in seq(0.05,0.30,0.05)) {
#     for (gam in seq(0.05,0.20,0.01)) {
#       tdr_2s_2by2(FILEPREFIX=fileprefix, 
#                   alphamax=0.20, alpha1max=0.99, alpha2max=0.99,alphamaxmax=0.99, 
#                   betamax=0.20, pwrmin=0.75,
#                   eta=0.99, gam=gam, expic=expic, ci=0.90,
#                   k=1)
...
# 
# if (P_LIST_ID == 2) {
#   fileprefix = 'TDR_alpha10_2s_2by2_alphaonly'
#   
#   for (expic in seq(0.05,0.30,0.05)) {
#     for (gam in seq(0.05,0.20,0.01)) {
#       tdr_2s_2by2(FILEPREFIX=fileprefix, 
...
# 
# if (P_LIST_ID == 3) {
# fileprefix = 'TDR_alpha20_2s_2by2_alphamax30'
# 
# for (expic in seq(0.05,0.30,0.05)) {
#   for (gam in seq(0.05,0.20,0.01)) {
#     tdr_2s_2by2(FILEPREFIX=fileprefix, 
...
# 
# if (P_LIST_ID == 4) {
# fileprefix = 'TDR_alpha10_2s_2by2_alphamax30'
# 
# for (expic in seq(0.05,0.30,0.05)) {
#   for (gam in seq(0.05,0.20,0.01)) {
#     tdr_2s_2by2(FILEPREFIX=fileprefix, 
